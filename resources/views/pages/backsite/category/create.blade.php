@extends('layouts.backsite.app')

@section('title', 'Tambah Kategori Baru')

@section('content')
    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Tambah Kaategori Baru</h1>
    </div>

    <!-- Form -->
    <form action="" method="post">
        <div class="form-group">
            <label for="kategori-nama">Nama Kategori</label>
            <input type="text" class="form-control" name="kategori-nama" id="kategori-nama" placeholder="Masukkan Judul Blog">
        </div>
        <div class="row my-3 mx-1">
            <button type="submit" class="btn btn-success p-2 mr-3">
                <i class="fa fa-save"></i>
                <span> Simpan</span>
            </button>
            <button type="reset" class="btn btn-danger p-2 mr-3">
                <i class="fa fa-save"></i>
                <span>Batalkan</span>
            </button>
        </div>
    </form>
@endsection
